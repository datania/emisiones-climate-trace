...
Este repositorio sincroniza los paquetes `country_packages` para el código ISO `ESP`, manteniendo copias reproducibles en Parquet comprimido con Zstandard y notas descriptivas publicadas en [climatetrace.org](https://climatetrace.org).
...
└── raw/
    ...
    │   └── DATA/
    │       ├── power/             # Archivos .parquet con series anuales (2015 en adelante)
    │       ├── industry/
...
Los CSV se convierten en Parquet zstd durante la descarga para reducir tamaño y evitar almacenar archivos intermedios. Los campos se guardan como texto para evitar errores de tipado en los CSV originales. Convierte columnas según sea necesario.

Los archivos Parquet (zstd) comparten columnas clave como `iso3_country`, `sector`, `subsector`, `start_time`, `end_time`, `gas` y `emissions_quantity`, lo que permite combinar emisiones por actividad, año y gas. Cada carpeta `ABOUT_THE_DATA` incluye resúmenes de metodología, cobertura geográfica y explicaciones sobre las métricas de confianza publicadas por Climate TRACE.
